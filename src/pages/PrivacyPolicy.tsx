...
export default function PrivacyPolicy() {
  ...
  useEffect(() => {
    async function loadSettings() {
      ...
      if (dbSettings && dbSettings.privacyPolicy) {
        setContent(dbSettings.privacyPolicy);
      } else {
        setContent('Privacy Policy will be updated soon.');
      }
      setLoading(false);
    }
    loadSettings();
  ...
  return (
    <main className="max-w-[800px] mx-auto px-8 py-16 min-h-[calc(100vh-200px)]">
      <h1 className="text-4xl font-bold text-primary tracking-tight mb-8">Privacy Policy</h1>
      {loading ? (
